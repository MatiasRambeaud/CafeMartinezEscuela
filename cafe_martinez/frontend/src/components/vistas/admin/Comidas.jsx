...
const Comidas = () => {
  ...
  const [form, setForm] = useState({ Nombre: "", Descripcion: "", ID_Menu: "" });
  ...
  const save = async (e) => {
    ...
    try { setLoading(true); setError("");
      ...
      if (!res.ok) throw new Error();
      setForm({ Nombre: "", Descripcion: "", ID_Menu: "" });
      setEditId(null);
      await load();
    ...
    <div style={{ fontFamily: "system-ui, -apple-system, Segoe UI, Roboto, sans-serif", color: "#1f2937" }}>
      ...
      <main style={{ maxWidth: 1000, margin: "0 auto", padding: "40px 16px" }}>
        ...
        <form onSubmit={save} style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8, marginBottom: 16 }}>
          ...
          <input name="ID_Menu" value={form.ID_Menu} onChange={onChange} placeholder="ID_Menu" style={input} />
          <button type="submit" style={btnPrimary}>{editId ? "Actualizar" : "Agregar"}</button>
          {editId && <button type="button" onClick={() => { setEditId(null); setForm({ Nombre: "", Descripcion: "", ID_Menu: "" }); }} style={btnGhost}>Cancelar</button>}
        </form>
          ...
          <tbody>
            {items.map((x) => (
              <tr key={x.ID_Comida}>
                ...
                <td style={td}>
                  <button onClick={() => { setEditId(x.ID_Comida); setForm({ Nombre: x.Nombre, Descripcion: x.Descripcion, ID_Menu: x.ID_Menu }); }} style={btnPrimary}>Editar</button>
