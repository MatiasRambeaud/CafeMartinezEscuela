...
const Menus = () => {
  ...
  const [form, setForm] = useState({ fecha: "", CHEF_ID: "" });
    ...
    <div style={{ fontFamily: "system-ui, -apple-system, Segoe UI, Roboto, sans-serif", color: "#1f2937" }}>
      ...
      <main style={{ maxWidth: 1000, margin: "0 auto", padding: "40px 16px" }}>
        ...
        <form onSubmit={save} style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 8, marginBottom: 16 }}>
          <input name="fecha" type="date" value={form.fecha} onChange={onChange} placeholder="Fecha" style={input} />
          <input name="CHEF_ID" value={form.CHEF_ID} onChange={onChange} placeholder="CHEF_ID" style={input} />
          <button type="submit" style={btnPrimary}>{editId ? "Actualizar" : "Agregar"}</button>
          {editId && <button type="button" onClick={() => { setEditId(null); setForm({ fecha: "", CHEF_ID: "" }); }} style={btnGhost}>Cancelar</button>}
        </form>
          ...
          <tbody>
            {items.map((x) => (
              <tr key={x.ID_Menu}>
                ...
                <td style={td}>
                  <button onClick={() => { setEditId(x.ID_Menu); setForm({ fecha: x.fecha?.slice(0,10), CHEF_ID: x.CHEF_ID }); }} style={btnPrimary}>Editar</button>
